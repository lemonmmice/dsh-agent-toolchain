...
export function summarizeNativeStacks(threads, o = {}) {
  ...
  // 没跑完时日志只有前半截：口径**必须自曝**，否则"2 条线程里没有驱动帧"会被读成"整机没有线程在驱动里"。
  const verdict = o.partial
    ? `⚠ 本次运行没有跑完${o.reason ? '（' + o.reason + '）' : ''}：下面的结论只覆盖日志里已写出的 ${list.length} 条线程，不能当全量结论。` + base
    : base

  return {
    threadCount: list.length, framesTotal, resolvedFrames, unresolvedFrames,
    runningInVendorDriver, graphicsStackThreads, partial: !!o.partial, verdict,
  }
}
