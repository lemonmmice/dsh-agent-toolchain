 ...
 *   name      参数名
 *   type      'string' | 'boolean' | 'number'（enum 也用 'string'，另给 enum 数组 —— 与 DSH 现状一致）
 *   enum      可选，取值数组
...
const P = {
  ...
  jevStateJson: { name: 'stateJson', type: 'string', required: true,
    zh: '要判断的 state JSON 字符串。内容会发送到 TypeSafe 远程 API；不要放入密码、token、完整客户数据或未脱敏截图。',
    en: 'JSON string for the decision state. It is sent to the remote TypeSafe API; omit passwords, tokens, raw customer data and screenshots.' },
  jevQuestionsJson: { name: 'questionsJson', type: 'string', required: true,
    zh: 'questions JSON 字符串：每项为 type=instruction/criteria 的 Choice、Score 或 Noul；建议一次批量放多个独立问题。',
    en: 'JSON string containing Choice, Score or Noul questions. Batch independent questions in one request.' },
  jevAllowRemote: { name: 'allowRemoteData', type: 'boolean', mcpDefault: false,
    zh: '明确允许把 state/questions 发到 TypeSafe。默认 false，未显式 true 时不联网。',
    en: 'Explicitly allow sending state/questions to TypeSafe. Defaults to false and never calls the network without true.' },
  jevTimeout: { name: 'timeoutMs', type: 'number', mcpDefault: 1200,
    zh: '单次请求超时（100–5000ms），默认 1200；超时只返回失败，不重试。',
    en: 'Request timeout (100-5000ms), default 1200; timeout fails open without retry.' },
  jevModel: { name: 'model', type: 'string', mcpDefault: 'jev-1.13.0',
    zh: '固定 Jev 版本，默认 jev-1.13.0；阈值依赖模型行为时不要用 latest。',
    en: 'Pinned Jev model id, default jev-1.13.0; pin a version when thresholds depend on behavior.' },
  jevUiGoal: { name: 'goal', type: 'string', required: true,
    zh: '希望客户端完成的 UI 目标；Jev 只在当前可见控件候选中做选择。',
    en: 'UI goal; Jev chooses only from the currently observed control candidates.' },
  jevUiInput: { name: 'inputValue', type: 'string', optional: true,
    zh: '可选输入值；Jev 不生成输入内容，只选择应写入的输入控件。敏感值不要放进远程 state。',
    en: 'Optional input value; Jev selects the field but never generates the value. Keep sensitive values out of remote state.' },
  jevUiMaxSteps: { name: 'maxSteps', type: 'number', mcpDefault: 8,
    zh: '最多观察/决策/动作轮数，默认 8，最大 20。',
    en: 'Maximum observe/decide/action turns, default 8, maximum 20.' },
  jevUiMaxControls: { name: 'maxControls', type: 'number', mcpDefault: 80,
    zh: '每轮发送给 Jev 的语义控件上限，默认 80，最大 120。',
    en: 'Maximum semantic controls sent per turn, default 80, maximum 120.' },
  jevUiAllowedNames: { name: 'allowedNamesJson', type: 'string', optional: true,
    zh: '可选 JSON 数组：仅允许这些精确控件名（菜单首行）或 AutomationId 进入候选集。',
    en: 'Optional JSON array restricting candidates to these exact names (first menu line) or AutomationIds.' },
  jevUiExpectSelected: { name: 'expectSelected', type: 'string', optional: true,
    zh: '用 UIA IsSelected 验证终点控件（精确 Name 或 AutomationId）；模型说 done 不等于验证通过。',
    en: 'Verify the terminal control using UIA IsSelected (exact Name or AutomationId); model done is not proof.' },
  jevUiTimeout: { name: 'timeoutMs', type: 'number', mcpDefault: 5000,
    zh: 'Jev 网络超时 100–5000ms，默认 5000；失败不重试。',
    en: 'Jev request timeout 100–5000ms, default 5000; no retries.' },
  jevUiConfidence: { name: 'confidenceThreshold', type: 'number', mcpDefault: 0.8,
    zh: '低于此置信度停止，不执行动作，默认 0.8。',
    en: 'Stop below this confidence without executing an action, default 0.8.' },
  jevUiAllowRemote: { name: 'allowRemoteData', type: 'boolean', mcpDefault: false,
    zh: '明确允许把脱敏 UI 状态发送到 TypeSafe；默认 false。',
    en: 'Explicitly allow sending sanitized UI state to TypeSafe; defaults to false.' },
  jevUiAllowEffects: { name: 'allowSideEffects', type: 'boolean', mcpDefault: false,
    zh: '明确允许执行点击/输入/滚动等真实副作用；默认 false。',
    en: 'Explicitly allow real UI side effects such as clicks, input, and scrolling; defaults to false.' },
  jevUiApproval: { name: 'approvalId', type: 'string', optional: true,
    zh: '可选：应用绑定的 UI 授权 ID。',
    en: 'Optional application-bound UI approval id.' },
  jevUiSession: { name: 'sessionId', type: 'string', optional: true,
    zh: '可选：授权所属会话 ID。',
    en: 'Optional session owning the UI approval.' },
  jevUiProcId: { name: 'procId', type: 'number', optional: true,
    zh: '可选：目标客户端 PID。',
    en: 'Optional target client PID.' },
  jevUiWinHandle: { name: 'winHandle', type: 'number', optional: true,
    zh: '可选：目标顶层窗口句柄。',
    en: 'Optional target top-level window handle.' },
  jevUiSecret: { name: 'secret', type: 'boolean', optional: true,
    zh: '输入值是否需要在本地动作证据中遮罩。',
    en: 'Mask the supplied input value in local action evidence.' },
}
...
export const REGISTRY = {
  jev_decide: {
    name: 'jev_decide',
    descZh: '调用 Jev 做**只读、建议性**的类型化判断：返回 Choice/Score/Noul 答案、概率/置信度和延迟，不执行 UI、构建、删除、重启或任何本地工具。默认不联网；必须显式 `allowRemoteData=true`，并设置 `TYPESAFE_API_KEY`。适合把多个独立的工具路由/证据筛选问题一次批量判断，不能替代现有授权门、快照新鲜度、编译结果或证据结论。Jev 主要训练语言是英文；中文、数字/日期、长无关上下文和对抗性文案必须在本地样本上校准。Triggers: Jev 判断 / 语义路由建议 / 批量筛选 / typed decision.',
    descEn: 'Make a **read-only, advisory** typed decision with Jev. Returns Choice/Score/Noul answers, probabilities/confidence and latency; it never executes UI, build, delete, restart or any local tool. Network use is disabled unless `allowRemoteData=true` and `TYPESAFE_API_KEY` is configured. Batch independent routing or evidence-selection questions in one request. It does not replace authorization gates, snapshot freshness, build results or evidence adjudication. Jev is primarily trained in English; calibrate Chinese, numeric/date, long-context and adversarial-content workloads locally. Timeouts fail open without retry. Input is JSON passed in `stateJson` and `questionsJson`.',
    params: [P.jevStateJson, P.jevQuestionsJson, P.jevAllowRemote, P.jevTimeout, P.jevModel],
  },
  ui_jev: {
    name: 'ui_jev',
    descZh: '用 Jev 对当前 UIA 控件清单做**受限、逐步的 UI 动作选择**：每轮先读取界面，再把脱敏后的窗口/控件候选发送给 Jev，Jev 只能返回候选 ID；真正的 click/setvalue/scroll 仍由 dsh-ui-drive 执行，并继续经过 allowSideEffects、snapshotId、approvalId、急停和应用策略。Jev 不读图、不生成任意输入文本，也不会绕过现有授权。默认 dry-run；必须显式 allowRemoteData=true 才发送 UI 状态，必须显式 allowSideEffects=true 才执行副作用。低置信度、歧义、**目标漂移**、超时和未知结果都会停止且不重试。每个动作都要求**恰好 1 个匹配**（requireUnique），并校验窗口句柄与元素矩形没变 —— 观察→决策→动作之间隔着几秒，界面换了就**在执行之前拒绝**（drift），而不是落到别的控件上。返回里还会如实带上"有多少控件根本没有 name/aid、因而够不到"。Triggers: 用 Jev 操作 UI / 语义点击 / typed UI action.',
    descEn: 'Use Jev for **bounded, stepwise UI action selection** over the current UIA control list. Each turn observes the UI, sends a sanitized window/control candidate set to Jev, and accepts only a candidate id; the existing dsh-ui-drive remains the executor and still enforces allowSideEffects, snapshot freshness, approvals, estop, and app policy. Jev does not see pixels, generate arbitrary input text, or bypass authorization. Dry-run is the default: allowRemoteData=true is required to send UI state, and allowSideEffects=true is required to execute effects. Low confidence, ambiguity, **target drift**, timeout, and unknown action results stop without retrying. Every action requires exactly one match (requireUnique) and verifies that the window handle and element rectangle have not changed — several seconds pass between observing, deciding and acting, so a screen that has changed since is refused **before anything executes** (drift) instead of landing on a different control. Results also report how many controls could not be addressed at all (no name and no AutomationId).',
    params: [P.jevUiGoal, P.jevUiInput, P.jevUiMaxSteps, P.jevUiMaxControls, P.jevUiConfidence, P.jevUiAllowedNames, P.jevUiExpectSelected, P.jevUiAllowRemote, P.jevUiAllowEffects, P.jevUiTimeout, P.jevModel, P.jevUiApproval, P.jevUiSession, P.jevUiProcId, P.jevUiWinHandle, P.jevUiSecret],
  },
  build_run: {
    name: 'build_run',
