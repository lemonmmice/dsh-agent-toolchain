...
export const REGISTRY = {
  ...
  "perf_clrevents": {
    "name": "perf_clrevents",
    "descZh": "从一个 .etl 汇总 **CLR 运行期事件**（GC 停顿 / 各代次数 / 托管堆 / 锁争用）—— 补的是 perf_probe（只测 UI 消息泵）与 perf_dump（冻结抓一瞬间）之间那条缝：「**GC 暂停导致的卡顿**」。⚠ **它只吃带 CLR provider 的 etl**：xperf/WPR 那条内核通道的参数是纯内核 flag，实测 276 MB 的 trace.etl 里**连 CLR provider 都没开**（e13c0d23 / a669021c 都不在）—— 要用它得先 `perf_trace(action=\"run\", seconds=N, clr=true)` 采一份。**本工具最重要的一条是三态，不是数字**：① etl 里没有 CLR provider ⇒ 报「**没采**」，**绝不报「GC 0 次」**；② provider 在、窗口内 0 次 GC ⇒ 那才叫「这段窗口确实没发生 GC」；③ 解码失败/超时/体积超限 ⇒ 一律写「**未知，不是 0**」。因为它默不作声地把「没采到」写成「没有」，agent 就会拿它去断言「客户端没有 GC 停顿」。停顿口径 = GC/SuspendEEStart → 其后第一个 GC/RestartEEStop（托管线程被冻结→恢复的真实时长），并给出最长的几段落在什么时刻（可与你看到的卡对上号；绝对时刻有约 1 分钟系统性误差，时长是差值不受影响）。Triggers: GC 停顿 / GC 次数 / 托管堆 / 锁争用 / CLR 事件 / 卡顿是不是 GC 引起的.",
    "descEn": "Summarize **CLR runtime events** from a .etl — GC pauses / per-generation counts / managed heap / lock contention. It fills the gap between perf_probe (UI message pump only) and perf_dump (one frozen instant): \"**the stutter is a GC pause**\". WARNING: it only works on an .etl that actually contains the CLR provider. The xperf/WPR kernel channel uses kernel-only flags and a measured 276 MB trace.etl had **no CLR provider at all** — capture with `perf_trace(action=\"run\", seconds=N, clr=true)` first. THE POINT OF THIS TOOL IS A THREE-STATE ANSWER, NOT A NUMBER: (a) no CLR provider in the etl => it says \"**never captured**\" and never \"GC count 0\"; (b) provider present but zero GC in the window => that genuinely means no GC happened; (c) decode failed / timed out / too large => \"**unknown, not 0**\". Getting this wrong is how an agent ends up asserting \"the client has no GC pauses\" from a trace that simply never collected them. Pause definition: GC/SuspendEEStart to the first following GC/RestartEEStop (the real time managed threads were frozen), plus the wall-clock positions of the longest pauses (absolute times carry a systematic ~1 minute offset from tracerpt's rendering; durations are differences and unaffected).",
    "params": [
      {
        "name": "etlPath",
        "type": "string",
  ...
  "ui_flow": {
    ...
    "descEn": "Run a whole UI verification sequence and collect evidence (find/read/windows/shot/wait/waitfor/expect are read-only; click/setvalue/key/type/drag need allowSideEffects=true). Steps: {action, name?, aid?, value?, keys?, ascii?, match?, index?, inAid?, inName?, waitFor?, state?, fromX?/fromY?/toX?/toY?, waitMs?, label?, expectEnabled?, expectMatch?}. waitFor on any action waits for a condition first (state=appear|gone|enabled|disabled); expect/waitfor count toward passed/failed. The sequence runs inside ONE PowerShell process (no per-step process start), so a 10-step flow takes ~1-2s. Every step output + screenshot is written to the evidence dir (steps.json); the returned transcript has passed/failed counts. For flows that must look at the screen between steps (login, captcha, branch on UI state), use ui_drive step by step instead.",
    "params": []
  },
}
