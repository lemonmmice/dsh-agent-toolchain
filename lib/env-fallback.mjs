...
export function envValues(names, { env = process.env, exec = defaultExec, fresh = false } = {}) {
  const hives = new Map()
  const readSnapshot = (file, args) => {
    const hive = args[1]
    if (!hives.has(hive)) {
      let output = ''
      try { output = exec(file, ['query', hive]) } catch { output = '' }
      hives.set(hive, output)
    }
    return hives.get(hive)
  }
  return Object.fromEntries(names.map((name) => [name, envValue(name, { env, exec: readSnapshot, fresh })]))
}
