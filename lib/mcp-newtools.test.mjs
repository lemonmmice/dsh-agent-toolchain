...
// 不是被测代码有问题）。改为断言**结构与语义**：必须有 hasRun 布尔；为 true 时必须带 logPath。
{
  ...
  check('build_status：hasRun=true 时带 logPath（可追溯）', s.hasRun !== true || typeof s.logPath === 'string',
...
// server.mjs 里手写了 if 链；action 取值来自 z.enum，两者若不同步会出现"schema 允许但没人处理"。
{
  const { readFileSync } = await import('node:fs')
  const src = readFileSync(join(repoRoot, 'mcp', 'server.mjs'), 'utf8')
  const block = src.slice(src.indexOf("'ui_live'"), src.indexOf('perf_probe'))
  const enumM = /action:\s*z\.enum\(\[([^\]]*)\]/.exec(block)
  const actions = enumM ? [...enumM[1].matchAll(/'([a-z-]+)'/g)].map((m) => m[1]) : []
  check('解析出 ui_live 的 action enum', actions.length === 5, JSON.stringify(actions))
