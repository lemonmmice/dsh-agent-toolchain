...
import { readFileSync, existsSync, mkdtempSync, mkdirSync, writeFileSync, rmSync, symlinkSync, rmdirSync } from 'node:fs'
...
// ---------------------------------------------------------------------------
{
  ...
  const execUser = (file, args) => {
    if (args[1] === 'HKCU\\Environment') return Buffer.from(
...
// ---------------------------------------------------------------------------
{
  ...
  try {
    ...
    check('★★ 生产路径的环境快照必须显式传 fresh: true',
      /envValues\(ENV_NAMES, \{ env, exec, fresh: true \}\)/.test(src), '')
...
{
  const dumpRoot = mkdtempSync(join(tmpdir(), 'tcs-tools-'))
  const configuredDump = join(dumpRoot, 'configured.exe')
  const configuredStack = join(dumpRoot, 'stack.exe')
  const defaultDump = join(dumpRoot, 'procdump.exe')
  const dacDir = join(dumpRoot, 'dac')
  mkdirSync(dacDir)
  writeFileSync(configuredDump, '')
  writeFileSync(configuredStack, '')
  writeFileSync(defaultDump, '')
  writeFileSync(join(dumpRoot, 'DumpStack.exe'), '')
  try {
    const configured = buildToolchainStatus({
      env: { DSH_NO_ENV_FALLBACK: '1', DSH_HANG_PROCDUMP: configuredDump, DSH_HANG_DUMPSTACK: configuredStack, DSH_HANG_DAC_DIR: dacDir },
      toolsRoot: dumpRoot, adminCheck: () => false,
    })
    check('dump 三件套使用本次注入环境，不泄漏宿主配置', configured.items.find((item) => item.key === 'procdump').value === configuredDump && configured.items.find((item) => item.key === 'dumpstack').value === configuredStack)
    const defaults = buildToolchainStatus({ env: { DSH_NO_ENV_FALLBACK: '1' }, toolsRoot: dumpRoot, adminCheck: () => false })
    check('toolsRoot 参数实际生效', defaults.items.find((item) => item.key === 'procdump').value === defaultDump && defaults.items.find((item) => item.key === 'dumpstack').value === join(dumpRoot, 'DumpStack.exe'))
    const calls = []
    let currentDump = configuredDump
    const registry = (file, args) => {
      calls.push([file, ...args])
      return args[1] === 'HKCU\\Environment' ? '    DSH_HANG_PROCDUMP    REG_SZ    ' + currentDump + '\r\n' : ''
    }
    envValue('DSH_HANG_PROCDUMP', { env: {}, exec: () => '    DSH_HANG_PROCDUMP    REG_SZ    C:\\stale\\procdump.exe\r\n', fresh: true })
    const fresh = buildToolchainStatus({ env: {}, exec: registry, toolsRoot: dumpRoot, adminCheck: () => false })
    const procdump = fresh.items.find((item) => item.key === 'procdump')
    check('dump 配置与其余状态一样 fresh，不读旧模块缓存', procdump.value === configuredDump && /用户级/.test(procdump.source), JSON.stringify(procdump))
    check('完整状态最多两次注册表探测，三件套不重复查询', calls.length === 2 && calls.every((args) => args.length === 3), JSON.stringify(calls))
    currentDump = defaultDump
    const next = buildToolchainStatus({ env: {}, exec: registry, toolsRoot: dumpRoot, adminCheck: () => false })
    check('连续状态调用可看到新 dump 注册表配置', next.items.find((item) => item.key === 'procdump').value === defaultDump && calls.length === 4)
    const running = buildToolchainStatus({ env: { DSH_NO_ENV_FALLBACK: '1', DSH_UI_PROC_NAME: 'ClientApp' }, procList: () => [{ image: 'ClientApp.exe', pid: 7632 }], toolsRoot: dumpRoot, adminCheck: () => false })
    check('精简进程表保留 PID/进程名匹配', running.items.find((item) => item.key === 'client').ok === true)
    const withSuffix = buildToolchainStatus({ env: { DSH_NO_ENV_FALLBACK: '1', DSH_UI_PROC_NAME: 'ClientApp.exe' }, procList: () => [{ image: 'ClientApp.exe', pid: 7632 }], toolsRoot: dumpRoot, adminCheck: () => false })
    check('配置进程名带 exe 后缀也匹配主目标', withSuffix.items.find((item) => item.key === 'client').ok === true)
  } finally {
