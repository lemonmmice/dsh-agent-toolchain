...
const { appendRecords, readAll, storeDir, readRetention } = await import('./capture-store.mjs')
...
// ---------- ③ 顺序守卫：**写/rename 必须在删之前** ----------
// ⚠ 说清楚本测试能证明什么、不能证明什么：
//   上面 ② 的失败是**注入在 `.tmp` 写入**上的（新实现才会走那条路），所以它证明的是
//   「**新实现**的重写失败不会丢数据」，并**不能**直接复现旧实现的缺陷（旧实现没有 .tmp 这一步）。
//   要防"有人把顺序改回去"，必须直接**盯住源码里的先后**：rename 必须在 rmSync 之前。
{
  const src = readFileSync(new URL('./capture-store.mjs', import.meta.url), 'utf8')
  const renameIdx = src.indexOf('renameSync(tmp, target)')
  const deleteIdx = src.indexOf('rmSync(file, { force: true })')
  ok('★ 源码守卫：原子 rename 出现在删除多余分片**之前**',
    renameIdx > 0 && deleteIdx > 0 && renameIdx < deleteIdx,
    `rename@${renameIdx} delete@${deleteIdx}（顺序反了就是"两者皆无"的窗口）`)
  // 反例守卫：删分片**必须**先过 `written.has(...)` 这道闸（无条件全删就是老写法）
  const loopAt = src.indexOf('for (const file of shardFiles()) {')
  const window = loopAt < 0 ? '' : src.slice(loopAt, loopAt + 260)
  ok('★ 源码守卫：删分片前必须有 `written.has(basename(file))` 闸门（无条件全删 = 老写法）',
    loopAt >= 0 && /written\.has\(\s*basename\(file\)\s*\)/.test(window),
    loopAt < 0 ? '没找到 shardFiles() 的循环' : JSON.stringify(window.slice(0, 110)))
}
