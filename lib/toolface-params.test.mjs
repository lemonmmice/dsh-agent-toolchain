...
import { checkRender, checkRenderShape, probePayloadRendered, synthFromSchema } from './output-render.mjs'
...
// ---------------------------------------------------------------------------
{
  ...
  // ⚠ 2026-09-23：`ui_jev` 两面新增 ⇒ 30 → 32。它是**整包转发**工具（DSH `jui().run(args)`、
  //   MCP `ju().run(args)`），按本节规则就该在这里**显式**对齐，而不是让上限悄悄变大。
  const SPREAD_EXEMPT_EXPECTED = 32
  ...
  // -------------------------------------------------------------------------
  {
    ...
    const outputIssues = []
    const unverifiable = []
    const renderIssues = []          // ★ 真返回值喂给真 render（F-056 同族的回归钉）
    const renderedTools = []
    const payloadIssues = []         // ★★ 载荷探针（F-059：渲染层把记录整个丢掉）
    const payloadNotProbed = []
    let payloadProbed = 0
    let withSchema = 0
    for (const [name, args] of READ_ONLY_CALLS) {
      ...
      // ★ 渲染层（F-056 同族）：宿主**把结果交给 agent 之前**会调 `output.render`。
      //   它不在 `execute` 的调用链上 —— 所以"execute 全绿 + schema 全绿"照样能有一支
      //   **渲染层是死的**工具（`api_capture_query` 的 `renderQuery is not defined` 就是这么活下来的）。
      //   这里用**真实**返回值调一次，形状也查（宿主只认 [{type,text}]）。
      if (out && typeof out.render === 'function') {
        try {
          const rr = out.render(args, result)
          const bad = checkRenderShape(rr)
          if (bad) renderIssues.push(name + ': ' + bad)
          else {
            renderedTools.push(name)
            // ★★ 载荷探针：**不抛 + 形状对** ≠ **数据给出来了**（F-059 的钉子）
            //   `failure_query` 只印「返回 2 条（库内合计 39…）」—— 记录正文一个字都没有；
            //   而它的 schema 是 `additionalProperties:true` 且**没声明属性** ⇒ 按 schema 合成的探针
            //   看不见它（合成值只能是 `{}`）⇒ **必须用这里的真实返回值探**。
            const text = rr.map((c) => (typeof c.text === 'string' ? c.text : '')).join('\n')
            const pr = probePayloadRendered(result, text)
            if (pr.applicable) {
              payloadProbed++
              if (!pr.ok) payloadIssues.push(name + ': ' + pr.detail)
            } else {
              payloadNotProbed.push(name)
            }
          }
        } catch (e) {
          renderIssues.push(name + ': render **抛了** ' + String(e && e.message ? e.message : e).split('\n')[0])
        }
      }
    ...
    // ★★ 渲染层（**不依赖上面那个校验器**，所以放在 else 外面）
    check('★ execute 冒烟真的覆盖到了 render（读到 0 个 = 这一格是空断言）', renderedTools.length >= 12, String(renderedTools.length))
    check('★★ 只读工具的**真实返回值**都能喂给它自己的 `output.render`（F-056 同族：0fad407 把 ' +
      '`api_capture_query` 的内联 render 换成了 `renderQuery(...)`，**而那个函数从没被写出来** ⇒ 宿主每次 ' +
      '`output.render failed: renderQuery is not defined`；而此前被执行过的只有 execute —— **render 从没人调**）',
      renderIssues.length === 0, renderIssues.join(' | '))
    // ★★ 载荷探针：**调得通**不等于**数据给出来了**（F-052 的 failure_query 版）
    check('★ 载荷探针真的探到了东西（读到 0 个 = 这一格是空断言）', payloadProbed >= 3, String(payloadProbed))
    check('★★ 只读工具的渲染层**不许把记录整个丢掉**（F-059：`failure_query` 只印「返回 2 条（库内合计 39…）」，' +
      '`execute` 返回的 rows 一个字都没渲染出来 —— 与 F-052 的 `memory_search` 是同一个病；' +
      '注意**按 schema 合成的探针看不见它**：该工具的 schema 是 `additionalProperties:true` 且没声明属性）',
      payloadIssues.length === 0, payloadIssues.join(' | '))
    if (payloadNotProbed.length > 0) {
      console.log('       （载荷探针**不适用**的只读工具 ' + payloadNotProbed.length + ' 个：' + payloadNotProbed.join(', ') +
        ' —— 它们的返回值里没有"非空对象数组"或元素里没有 ≥12 字符的字符串 ⇒ **未证 ≠ 没问题**）')
    }
    // 反向自证：载荷探针**不是恒绿**的 —— 给它一个"只印条数、把记录丢掉"的假渲染，必须判出来
    const fakeRows = [{ task: '这是一个足够长的任务名用来当探针', description: '这是描述' }, { task: '第二条足够长的任务名' }]
    const prBad = probePayloadRendered(fakeRows, '返回 ' + fakeRows.length + ' 条')
    const prGood = probePayloadRendered(fakeRows, '返回 2 条\n' + fakeRows[0].task)
    const prNone = probePayloadRendered({ total: 1 }, '返回 1 条')
    check('★ （反向自证）载荷探针：只印条数 ⇒ 判出；印了记录 ⇒ 通过；没有记录数组 ⇒ 明确"不适用"',
      prBad.applicable === true && prBad.ok === false && prGood.ok === true && prNone.applicable === false,
      JSON.stringify({ bad: prBad, good: prGood, none: prNone }))
  ...
  // -------------------------------------------------------------------------
  // 1f. ★★ **每一支工具的 `output.render` 都要能被调用**（r56 新增，**用户重启宿主后真机撞出来**）
  //
  // 1e 只覆盖了 17 个**只读**工具；而 render 是**所有**工具都要过的一关
  // （宿主把结果交给 agent 之前一律调它）。副作用工具没法执行 execute，但**不需要执行也能验渲染**：
  //   按工具自己的 `output.schema` 合成一个**合法返回值**，直接调 `output.render`。
  //
  // 现场（2026-09-14）：用户重启宿主后第一次调 `api_capture_query`：
  ...
  //   **而 `renderQuery` 从没被定义/导入**（本文件注释还写着"渲染逻辑在可测的 lib/query-view.mjs 里"）。
  //   它此前**被 F-055 的 schema 错挡在前面**（宿主是先验 schema 再渲染）—— 修好 schema、重启之后才露头。
  //   ⇒ 两个 bug **叠在同一支工具上**，一个盖住另一个，这就是"修好一个才看见下一个"的真实样子。
  //
  // 顺带抓出第二条（同一族但性质不同）：`dsh-memory/memory_search` 的 schema 写 `hits: { type: 'array' }`
  //   —— 按 JSON Schema 语义，**元素可以是任何东西（含 null）**，而它的 render 是"按元素是对象"写的
  //   （读 `h.file`）⇒ 一个**符合它自己 schema** 的值就能把它打崩：
  //     Cannot read properties of null (reading 'file')
  //   真实生产者永远给对象，所以这**不是线上已发生的崩溃**，而是**契约比实现宽**：
  //   哪天返回里混进一个 null，崩的就是渲染层，而且没有任何一关会先红。修法是**把 schema 收紧到写实**。
  // -------------------------------------------------------------------------
  {
    const RENDERED_FLOOR = 47          // 标定值：47 个工具**全部**声明了 output.render
    const bad = []
    const rendered = []
    for (const [name, out] of toolOutputs) {
      const v = checkRender({ name, output: out })
      if (v.status === 'no-render') continue
      rendered.push(name)
      if (v.status !== 'ok') bad.push(name + ' [' + v.status + '] ' + v.detail)
    }
    check('★★ 声明了 `output.render` 的工具数**只许变大**（从 ' + RENDERED_FLOOR + ' 掉下来 = 覆盖被悄悄缩小）',
      rendered.length >= RENDERED_FLOOR, String(rendered.length) + '（标定值 ' + RENDERED_FLOOR + '）')
    check('★★ **每一支**工具的 `output.render` 都能吃下"符合它自己 output.schema 的值"、且返回 [{type,text}]' +
      '（合成值口径见 lib/output-render.mjs；`renderQuery is not defined` 与 `reading \'file\' of null` 都栽在这里）',
      bad.length === 0, bad.join(' | '))
    // 反向自证：这条判据**不是恒绿**的 —— 现场拿一个坏 render 喂进去，必须被判出来。
    const probe = checkRender({ output: { schema: { type: 'object', properties: {} }, render: () => { throw new Error('探测用的假抛错') } } })
    check('★ （反向自证）坏掉的 render 确实会被判出来', probe.status === 'throws' && /探测用的假抛错/.test(probe.detail), JSON.stringify(probe))
    const probe2 = checkRender({ output: { schema: { type: 'object', properties: {} }, render: () => [{ type: 'text' }] } })
    check('★ （反向自证）返回缺 text 的内容项也会被判出来', probe2.status === 'bad-shape', JSON.stringify(probe2))
    const probe3 = checkRender({ output: { schema: { type: 'null' }, render: () => [{ type: 'text', text: 'ok' }] } })
    check('★ （反向自证）形状对的时候确实是 ok（不是恒红）', probe3.status === 'ok', JSON.stringify(probe3))
    // 合成器自身也要自证：它必须**跟着 schema 走**，不许脑补字段（脑补会把契约问题洗成通过）
    // ⚠ 最后一条是**故意**的：`{type:'array'}` 没声明 `items` ⇒ 按 JSON Schema 语义元素可以是任何东西
    //   ⇒ 合成 `[null]`（**不是**空数组）。这正是把 `memory_search` 打崩的那个值 —— 修法是收紧 schema。
    check('★ （反向自证）合成值跟着 schema：oneOf 取第一支 / enum 取第一个值 / 声明的属性都在 / 数组给一个元素 / 未声明 items 的元素可以是 null',
      synthFromSchema({ oneOf: [{ type: 'string' }, { type: 'number' }] }) === 'x' &&
      synthFromSchema({ type: 'string', enum: ['a', 'b'] }) === 'a' &&
      JSON.stringify(synthFromSchema({ type: 'object', properties: { a: { type: 'integer' }, b: { type: 'boolean' } } })) === '{"a":1,"b":false}' &&
      JSON.stringify(synthFromSchema({ type: 'array', items: { type: 'string' } })) === '["x"]' &&
      JSON.stringify(synthFromSchema({ type: 'array' })) === '[null]',
      'oneOf=' + JSON.stringify(synthFromSchema({ oneOf: [{ type: 'string' }] })) +
      ' 无items数组=' + JSON.stringify(synthFromSchema({ type: 'array' })) +
      ' obj=' + JSON.stringify(synthFromSchema({ type: 'object', properties: { a: { type: 'integer' } } })))
    console.log('       （render 冒烟：' + rendered.length + ' 个工具（**全部**）；1e 那条用的是**真实返回值**，' +
      '这条用的是**按 schema 合成的值** —— 两条都留着，一条管真、一条管全）')
  }
  ...
  // -------------------------------------------------------------------------
  {
    ...
    //     r61（D.3）：分析器现在把这条原因**机器化输出**为 `args-reconstructed-generically(Object.entries…)`，
    //     不再与"真·没读到"（`no-field-read-in-callee-function-body`）糊成同一句 —— 状态仍是"未查"，
    //     但原因写准了（跟不过是因为键名可能被 `FILTER_PARAM_ALIASES` 改写，跟过去也不可信；见 lib/callee-wiring.test.mjs 的 D.3 节）。
    //   · `ui_jev`（2026-09-23 新增，两面）：**跟得到被调方** —— `jui().run(args)` / `ju().run(args)`
    //     都 resolve 到 `lib/jev-ui.mjs` 的 `run`（实测 resolveCallee 返回 ok，depth=1）。证不到的是
    //     **读点形态**：`run(input)` 一进来就把整包展开进本地对象
    //     （`const options = { …默认值, ...input }`），之后全部读 `options.P`，另有 5 个走
    //     `options[field]` 动态索引（maxSteps / maxControls / timeoutMs / procId / winHandle）——
    //     对 `input` 本体而言，函数体里**一个字段读都没有** ⇒ 两面**各 16/16 未证**。
    //     这是**分析器能力边界**（读点确实存在，只是长在重建后的对象上），不是跨语言，也不是"参数没被读"。
    //     ⚠ 刻意**不**给它钉参数级地板：证到数是 0，任何地板都是空断言（空断言比没有断言更坏）。
    const KNOWN_UNWIRED = [
      'DSH api_capture_query', 'DSH ui_act', 'DSH ui_drive', 'DSH ui_jev', 'DSH ui_observe',
      'MCP ui_act', 'MCP ui_drive', 'MCP ui_jev', 'MCP ui_observe',
    ]
