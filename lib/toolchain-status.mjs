...
const ENV_NAMES = [
  'DSH_UI_PROC_NAME', 'DSH_UI_CLIENT_EXE', 'DSH_BUILD_CLIENT_PROC', 'DSH_UI_PROC_CANDIDATES',
  'DSH_HANG_SRC_ROOT', 'DSH_PERF_SRC_ROOT', 'DSH_PERF_SYMBOL_PATH',
  'DSH_HANG_PROCDUMP', 'DSH_PERF_PROCDUMP', 'DSH_HANG_DUMPSTACK', 'DSH_PERF_DUMPSTACK',
  'DSH_HANG_DAC_DIR', 'DSH_PERF_DAC_DIR', 'DSH_PERF_EVIDENCE_DIR', 'DSH_HANG_EVIDENCE_DIR',
]

const defaultProcList = () => {
  const script = '$ErrorActionPreference="Stop"; [Console]::OutputEncoding=[System.Text.UTF8Encoding]::new(); @([Diagnostics.Process]::GetProcesses() | ForEach-Object { [pscustomobject]@{image=$_.ProcessName+".exe";pid=$_.Id} }) | ConvertTo-Json -Compress'
  ...
  const procs = Array.isArray(rows) ? rows : [rows]
  if (!procs.every((row) => row && typeof row.image === 'string' && Number.isInteger(row.pid) && row.pid >= 0)) throw new Error('invalid_process_list')
  return procs
}

function parseProcessList(txt) {
 ...
 */
export function buildToolchainStatus({ deep = false, env, exec, toolsRoot, procList, adminCheck } = {}) {
  const environment = envValues(ENV_NAMES, { env, exec, fresh: true })
  const ev = (name) => environment[name]
  ...
  if (candidates.length === 0) {
    ...
    try { procs = parseProcessList(listProcs()) } catch { procs = null }
    if (procs === null) {
      add('client', '目标客户端进程', '进程列表查询失败（无法判断是否在运行）',
        procEnv.source || 'missing', false,
        '**没能执行进程列表查询** —— 这是"没查成"，**不等于"客户端没开"**；本应扫描：' + scannedDesc,
        '在目标主机上确认 Windows PowerShell 进程查询可用后复验（当前配置 DSH_UI_PROC_NAME=' + (procName || '未配置') + '）')
    } else {
      ...
      const primaryHit = procName ? hitOf(procBaseName(procName)) : null
      if (primaryHit) {
        // (b) 配置名对、进程在跑 ⇒ 正常报 pid
  ...
  try {
    const tools = resolveDumpTools({
      ...
      toolsRoot: toolsRoot || join(homedir(), '.dsh-agent-toolchain', 'tools'),
      env: Object.fromEntries(ENV_NAMES.map((name) => [name, ev(name).value])),
    })
    for (const [key, origin] of Object.entries(tools.origins)) {
      const source = environment[origin]?.source
      if (source === 'user') tools.origins[key] += '（取自用户级环境变量）'
      if (source === 'machine') tools.origins[key] += '（取自机器级环境变量）'
    }
  ...
  // ---- 5. 管理员（ETW 内核会话的前提）----
  {
    let isAdmin = false
    try {
      if (adminCheck) isAdmin = adminCheck() === true
      else {
        execFileSync('net', ['session'], { windowsHide: true, timeout: 5000, stdio: ['ignore', 'ignore', 'ignore'] })
        isAdmin = true
      }
    } catch { isAdmin = false }
