...
// ---------------------------------------------------------------- budgetText
{
  ...
  check('截后是原文前缀（不编造内容）', big.startsWith(r.text) && r.text.length < big.length)
  ...
  const cjk = budgetText('测'.repeat(1000), 50) // 1000 tokens → 截到 ≤50
  check('CJK 文本也能收敛到预算内', cjk.truncated === true && cjk.keptTokens <= 50, 'kept=' + cjk.keptTokens)
  check('CJK 截后仍是前缀', '测'.repeat(1000).startsWith(cjk.text))
