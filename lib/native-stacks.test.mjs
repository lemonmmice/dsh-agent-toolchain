...
`).threads)
    ...
    const shallow = summarizeNativeStacks(Array.from({ length: 10 }, (_, i) => ({ tid: 100 + i, mode: 'x86', frames: ['wow64cpu!TurboDispatchJumpAddressEnd+0x515'] })))
    check('④ ★中位数 1 帧 → 必须判为"栈没回溯出来"（x86 回溯要 pdb），且不许下任何结论',
      shallow.walkLooksBroken === true && /明显不完整/.test(shallow.verdict) && /PDB/.test(shallow.verdict), shallow.verdict)
    // 实测形态：94 条线程里 83% 只有 1 帧，但少数线程帧多，**平均值 2.61 会把判据糊弄过去** → 所以要按中位数/占比判
    const skewed = summarizeNativeStacks([
      ...Array.from({ length: 9 }, (_, i) => ({ tid: 300 + i, mode: 'x86', frames: ['wow64cpu!TurboDispatchJumpAddressEnd+0x515'] })),
      { tid: 399, mode: 'x86', frames: Array.from({ length: 40 }, (_, k) => `m!f${k}+0x1`) },
    ])
    check('④ ★★少数线程帧多也不能掩盖"没回溯出来"（中位数 1 / ≤1帧占比 90% → 判 broken）',
      skewed.walkLooksBroken === true && skewed.medianFramesPerThread === 1 && skewed.shareThreadsWithOneFrameOrLess === 0.9,
      JSON.stringify({ b: skewed.walkLooksBroken, m: skewed.medianFramesPerThread, s: skewed.shareThreadsWithOneFrameOrLess, avg: skewed.avgFramesPerThread }))
    const deep = summarizeNativeStacks(Array.from({ length: 10 }, (_, i) => ({ tid: 200 + i, mode: 'x86', frames: ['ntdll!a+0x1', 'kernel32!b+0x2', 'msvcr100!c+0x3'] })))
    check('④ 正常深度的栈不会被误报成"不完整"', deep.walkLooksBroken === false && deep.avgFramesPerThread === 3, JSON.stringify({ b: deep.walkLooksBroken, a: deep.avgFramesPerThread }))
  ...
  // ---------------------------------------------- ⑤ 命令串（两个坑）
  {
    ...
    check('⑤ 默认全量走 ~*k，帧数明确使用十进制', /~\*k/.test(one) && /\.kframes 0n12;/.test(one) && /q$/.test(one), one)
    check('⑤ --frames 64 不能被 cdb 当成十六进制 100', buildUnwindCommands({ frames: 64, switchWow64: false }).startsWith('.kframes 0n64;'), buildUnwindCommands({ frames: 64, switchWow64: false }))
