...
## 为什么这一块**不做** Rust 化（2026-09-20 实测）

常被问"堆栈解析能不能用 Rust 加速"。本仓的规矩是[先量再判](./rust-rewrite-scope.md)，所以先跑基准
（本机 Node v24.15.0，取 7 次中位数）：

| 规模 | 日志 | `parseNativeStacks` | `summarizeNativeStacks` | 合计 |
| --- | --- | --- | --- | --- |
| **真实**（有符号档） | 96 KB / 94 线程 / 1099 帧 | 1.13 ms | 0.69 ms | **1.8 ms** |
| 真实（无符号档） | 27 KB / 245 帧 | 0.19 ms | 0.16 ms | 0.35 ms |
| 合成 2,256 帧 | 0.1 MB | 1.41 ms | 1.52 ms | 2.9 ms |
| 合成 5 万帧 | 2.6 MB | 27.3 ms | 20.4 ms | 47.7 ms |
| 合成 20 万帧 | 10.4 MB | 116 ms | 52 ms | 168 ms |
| 合成 100 万帧 | 51.5 MB | 578 ms | 233 ms | 811 ms |

同一台机器上的对照量级：

- **Node 启动**约 **60 ms**（中位数 59.8，min 54）—— 比真实日志的解析本身贵 **30 倍**；
- 整条 `--from-log` CLI（启动 + 读文件 + 解析 + 打印）**84 ms**；
- cdb 无符号档 **0.2 ~ 0.3 s**；cdb 有符号档首次跑（下 pdb，网络受限）**约 700 s**。

⇒ 真实场景里解析占 cdb 墙钟的 **0.6%（vs 无符号档）～ 0.00026%（vs 有符号档）**，
占 CLI 自身 84 ms 的约 **2%**。Rust 化最多把 1.8 ms 压到 0.1 ms 量级（**省 1.7 ms**），
代价是一块新的 Node-API 原生模块（构建脚本 + CI + 跨平台产物 + 部署守卫）——
按本仓 `rust-rewrite-scope.md` 的同一口径：**投入产出不成立**。

**时间真正花在哪**：① cdb（调试器引擎本身，已经是原生 C++ 在干活）；② **从符号服务器下 pdb**（网络）。
这两块 Rust 都救不了。要提速就动这两处：把符号缓存热起来（跨运行共享），
或用 `--no-symbols`（**仅** 64 位目标 / 探活，见上）。

**什么时候才值得再看一眼**：如果哪天要把 `kb`/`kv`（带寄存器/局部变量）也存下来，
日志会到几十 MB 级 —— 那时 47 ~ 811 ms 才有意义，而且**优先做 JS 层的小改动**
（一次小写化 + 前缀集合判定），不是直接上原生模块。
**唯一够格让 Rust 上场的理由是"不想依赖 cdb/WinDbg"** —— 那意味着自己写 minidump 解析 +
x86 FPO 回溯 + 符号服务器客户端，那是**功能重写**（量级完全不同），不是"加速解析"。
